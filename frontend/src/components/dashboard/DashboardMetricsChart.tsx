...
const createChartData = (totalRevenue: number, dailyOpdVisits: number): ChartDataPoint[] => {
  const weeklyRevenue = Math.max(totalRevenue, 0)
  const weeklyVisits = Math.max(dailyOpdVisits * 7, 0)

  return labels.map((day, index) => ({
    day,
    revenue: Math.round(weeklyRevenue * distribution[index]),
    visits: Math.round(weeklyVisits * distribution[index]),
  }))
}
  ...
  return (
    <div className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
      <div className="mb-4">
        ...
        <p className="text-sm text-slate-600">Revenue and OPD visits distribution for quick trend insight.</p>
      </div>
